...
# Fall 2020
#
# Simulates the an a* search to find the ideal path from a user generated 
#       pancake stack to a goal. Also conatins the implementation and 
#       definition or our node class
#
...
class Node:
    ...
    # Parameters: A list of ints that contains a pancake order, a parent node,
    #             an int with the path cost for the node and an int with the 
    #             The heuristic cost
    # Returns: nothing
    # Does: Gives the child a parent 
    def __init__(self, pancake_order, node_parent , path_cost,
                 heuristic_cost) :
        
        #Let node keep track of parents and it's children
        self.parent          = node_parent
        self.children        = []
        
        #Node has a pancake stack
        self.cake_stack      = pancake_order 
        self.total_cost      = ( path_cost + heuristic_cost )
        self.backwards_cost  = path_cost
        self.forward_cost    = heuristic_cost
    
    # function name: add_new_child
    # Parameters: A node that will become a new child of our current node
    # Returns: Nothing
    # Does: Adds a node to the back of our current nodes child list 
    def add_new_child (self, new_child) :
        self.children.append( new_child )
    
    # function name:  Less than overload operator
    # Parameters: A node to compare to our current one
    # Returns: True if our self node's total_cost is less than the other 
    #          nodes total_cost cost 
    # Does: Comapres two nodes if a less was used on a Node Object
...
# function name: heuristic_function
# Parameters: list of ints which represents the stack of pancakes
# Returns: nothing
# Does: Calculates the heuristic value of any given stack
def heuristic_function( pancake_stack ):
    ...
    for i in range(0,10):
        ...
        #Add to the heuristic if the plate and the bottom pancake differe
        #by more than one
        if i == 0 :
            if pancake_stack[i] != 10 :
                heuristic_total += 1
            
        #Add to the heuristic if pancake bellow our current one differs by
        #more than one        
        else:
            if ( pancake_stack[i] == pancake_stack[i - 1] - 1 or 
                 pancake_stack[i] == pancake_stack[i - 1] + 1  ) :
                continue
            else :
                heuristic_total += 1
...
# Does: Creates new values for the path cost, heuristic value, and pancake
...
def make_node( node_parent, flip_index ):
    ...
    heuristic_cost   = heuristic_function( new_stack )
...
#       through a 
def a_star(pancake_stack, frontier) :
...
if __name__ == "__main__":
    ...
    result = a_star(pancake_stack, frontier)
